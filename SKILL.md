...
# Alfred adaptation: prefer the repo's pinned 3.12 venv (docling/torch have no Homebrew py3.14 wheels).
# SCRIPT_PATH is <repo>/scripts/extract.py, so the venv lives at <repo>/.venv.
if [ -z "${PYTHON_BIN:-}" ]; then
  _bts_venv="$(cd "$(dirname "$SCRIPT_PATH")/.." 2>/dev/null && pwd)/.venv/bin/python"
  [ -x "$_bts_venv" ] && PYTHON_BIN="$_bts_venv"
fi
